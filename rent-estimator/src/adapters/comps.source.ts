...
export class MockCompsSource implements CompsPort {
  ...
  async searchComps(params: {
    ...
    // Filter by days back
    if (params.daysBack) {
      filtered = filtered.filter(comp => (comp.daysOld ?? 0) <= params.daysBack!);
    }
    ...
    if (params.propertyType) {
      filtered = filtered.filter(comp => comp.propertyType === params.propertyType);
    }
    ...
    if (params.beds !== undefined) {
      filtered = filtered.filter(comp => 
        comp.beds === undefined || 
        Math.abs(comp.beds - params.beds!) <= 1
      );
    }
    ...
    if (params.baths !== undefined) {
      filtered = filtered.filter(comp => 
        ...
        Math.abs(comp.baths - params.baths!) <= 1
      );
    }
    ...
    if (params.lat !== undefined && params.lng !== undefined && params.radiusKm) {
      filtered = filtered
        .map(comp => ({
          ...comp,
          distanceKm: comp.lat && comp.lng ? 
            this.calculateDistance(params.lat!, params.lng!, comp.lat, comp.lng) : 
            undefined
        }))
        .filter(comp => comp.distanceKm === undefined || comp.distanceKm <= params.radiusKm!);
    }
